...
**TS layer (`src/`).** `zstd-wasm-decoder.ts` wraps the WASM module with `ZstdDecoder` (sync `decompressSync` for buffers ≤ ~9.4 MB after expected-size hint check, falls back to `decompressStream` otherwise). `zstd-wasm-encoder.ts` wraps the compress side (`ZstdEncoder`, level 1 only; other levels throw). `index.node.ts`, `index.web.ts`, `index.web.inlined.ts` are environment-specific entrypoints — they differ only in how they obtain the `WebAssembly.Module` (filesystem, fetch, base64-inlined, etc.). `shared.ts` holds env-agnostic decoder glue (`ZstdDecompressionStream`, `setupZstdDecoder`, the decoder pool); `encoder-shared.ts` holds the analogous encoder glue. `types.d.ts` and `dist/types/` (generated) are the public type surface.
