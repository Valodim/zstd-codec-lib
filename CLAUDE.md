...
`zstd-wasm-codec` — a tiny Zstandard codec (decoder + level-1 compressor) compiled from the upstream `facebook/zstd` C source (in `vendor/zstd`) to WebAssembly, plus a thin TypeScript wrapper. pnpm workspace; the only package today is `packages/zstd-wasm-codec`.
...
`packages/zstd-wasm-codec/Makefile` is the source of truth for WASM flags (`size` / `perf` targets, `regenerate-amalgamated`, `check-tools`). `make` from inside that package == `pnpm run build:wasm`.
